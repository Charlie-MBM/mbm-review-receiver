...
8. **Hint API limits we confirmed:** cannot archive/deactivate a patient (no field, endpoints 404 — 200s on unknown fields are ignored); cannot create plans (list returns only id/name/plan_type); does not expose appointment *type* on the appointment object (title is null). Plan/appointment-type config is UI-only.
...
|---|---|---|---|---|
| **MBM Consult Intake Poller** | 5:30 PM daily | `webhook\run_consult_intake.bat` | `send_consult_intake.py` | Next-day reminders/confirmations; **auto-creates a Hint patient** for new online-booking prospects; **detects no-shows + sends Touch 1** |
...
| **MBM-Prospect-Form-Alert** | 9:00 AM daily | `webhook\run_prospect_alert.bat` | `send_prospect_form_alert.py` | Texts James+Charlie the list of today's consult prospects who haven't submitted their HIPAA intake |
