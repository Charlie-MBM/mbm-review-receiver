...
## Review click-tracker × Cloudflare — migration gotchas (documented 2026-07-24, root-caused + fixed 2026-07-25)

The July 2026 Cloudflare account migration (new account `95619789467f5b8aa49e44428e1ed443`, zone `cb3c17e57f8c566dc2e4139b02f57b90`, workers.dev subdomain `charlie-956`; the old `charlie-6c7` subdomain is DEAD) left several tripwires around `/api/review-clicked`. All are resolved; the history matters so nobody re-breaks it.

1. **Custom-domain config gap.** `wrangler.jsonc` MUST declare `routes: [{"pattern":"mtbakermedical.com","custom_domain":true}]` or every wrangler/CI deploy strips the custom domain and mtbakermedical.com falls back to a 404 host (caused the click-tracker 404 flapping, 2026-07-24). Never remove it.
2. **Cloudflare bot protection blocks the poller.** A python-requests default User-Agent gets CF error 1010 (banned browser signature). `fetch_clicked_fname_hashes()` in `send_review_requests.py` sends a real browser UA + a `?_cb=<timestamp>` cache-buster. Keep both.
3. **KV account move.** `REVIEW_CLICKS` was recreated (empty) in the new account, id `dd21959f827d4a54a07e6b915573d633`, and is bound in `wrangler.jsonc` alongside `BOOK_KV` (`eb69b832...`). `CLICK_TRACKER_TOKEN` was re-set as a Worker secret on the new account. NOTE: one botched `wrangler secret put` left a junk secret named `CLICK_TRACKER_TOKEN<value>` (name and value concatenated) on the Worker — harmless, delete when convenient. Click history from before the migration is gone (list restarted empty).
4. **Review flow.** The SMS link `/r?n=Name` writes the tap directly to REVIEW_CLICKS (SHA-256 of the lowercased first name — never the plain name) then 302s straight to the Google review form (`search.google.com/local/writereview?placeid=ChIJm8NquAOjhVQRCanaH1z4viE`). Review SMS copy is neutral, "Reply DONE" removed, per Google's 2026 no-coaching policy.
5. **The "CI deploy drops the KV binding" mystery — TWO stacked causes, both fixed:**
   - *Cause A (config drift):* at migration the REVIEW_CLICKS binding was removed from the COMMITTED `wrangler.jsonc` while Charlie's local working tree had it re-added. CI deploys read repo state → shipped without the binding; manual `wrangler deploy` from the laptop included it. Fixed when the binding re-add was committed. The GitHub secrets (`CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID`) were NEVER the problem — they provably pointed at the new account throughout (verified 2026-07-25 via CF version API: every CI deploy landed on the new account carrying both KV bindings).
   - *Cause B (runtime, the sneaky one):* the Worker's fetch-signature `env` is INTERMITTENTLY undefined per-isolate (the known quirk `resolveEnv` in `src/book/api.ts` exists for). The review-clicked handler read `env` raw, so on affected isolates `env.REVIEW_CLICKS` came back undefined even though the binding WAS attached → the handler forwarded to the dead old-account sidecar → Cloudflare 404/1042 proxied back → poller logged an error and silently skipped suppression. This made the bug look deploy-correlated when it was actually isolate-lottery. Fixed 2026-07-25: the handler now resolves env via `getBookEnv()` and the dead sidecar forward was replaced with a loud 503 `{"error":"kv_unavailable"}` (commits `04980db..6328baa`). Verified 10/10 stable 200s across isolates post-fix.
6. **The interim rule ("manually `wrangler deploy` after every push") is RETIRED** — CI deploys are trustworthy since Cause A+B were fixed. If `/api/review-clicked` ever regresses: authed GET returning HTML/404 = env-resolution or domain-route problem; 401 JSON on a no-auth GET = healthy handler; 503 `kv_unavailable` = binding genuinely missing from the deploy (check `wrangler.jsonc` kv_namespaces and the CF version API).
7. **Known cosmetic artifact:** one synthetic test hash sits in REVIEW_CLICKS from the 2026-07-25 write-path verification (`fname:0589d2c1...`, SHA-256 of "mbmkvtest"). The API token lacks KV-delete scope; remove via CF dashboard → KV → REVIEW_CLICKS if the +1 in click counts ever matters.
