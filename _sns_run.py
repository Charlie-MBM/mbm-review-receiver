#!/usr/bin/env python3
"""Fresh-inode verification copy of send_nurture_sequence.py (T5d). The sandbox
mount caches a stale view of the overwritten canonical file; this copy lets the
sandbox run the real logic. Canonical file = send_nurture_sequence.py (host)."""
import argparse
import json
import sys
...
MEMBERSHIP_DENYLIST = {"mem-zhNHV8snambF"}  # Dan I. - ratified not-for-nurture
    ...
    if not state.get(mem_id, {}).get("enrolled_at"):
        ...
        try:
            ...
            if cdt.date() >= datetime.now(cdt.tzinfo).date():
                # Check review de-confliction before wait: a review-conflict defer
                # should take precedence so it's recorded as defer not wait.
                if E.review_asked_today(pat_id):
                    plan.update(action="defer", day=0,
                                reason="review poller asked this patient today; defer 1 day")
                    return plan
